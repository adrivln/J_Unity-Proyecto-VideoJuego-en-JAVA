...
public class Sprite implements Serializable, Comparable<Sprite> {
	...
	private Sprite(String name, int x1, int y1, int x2, int y2, double angle, Field f) {
		...
		this.f = f;
		if (x2 < x1 || y2 < y1) {
		//	System.err.println("ERROR. SPRITE INITIALIZED WITH NEGATIVE LENGHT/HEIGHT"); tret pq ja ho vull aixi 
		}
		this.layer = 0;
		if (f != null)
	 ...
	 * @param path  path to the image of that sprite, as a relative path
	 */
	protected Sprite(String name, int x1, int y1, int x2, int y2, double angle, String path) {
